...
import { WORKTREE_NAME_PATTERN } from './shared.ts'
...
export function validateWorktreeName(name: string): string {
  if (!WORKTREE_NAME_PATTERN.test(name)) {
    throw new Error(
      'invalid worktree name: use 1-64 characters, letters/digits first, '
 ...
 */
export function excludeContains(content: string | undefined, entry: string): boolean {
 ...
 */
export function withExcludeEntry(content: string | undefined, entry: string): string {
