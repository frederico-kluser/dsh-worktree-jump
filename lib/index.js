...
import { isAbsolute, isAbsolute as isAbsolute$1, join, join as join$1, relative } from "node:path";
...
function planWorktree(repoRoot, gitDir, name, configRoot) {
	const worktreeRoot = configRoot ?? join$1(repoRoot, ".worktrees");
	const worktreePath = join$1(worktreeRoot, name);
	const rel = relative(repoRoot, worktreeRoot);
	return {
		worktreeRoot,
		worktreePath,
		branch: name,
		excludeEntry: rel !== "" && !rel.startsWith("..") && !isAbsolute$1(rel) ? toPosix(rel) + "/" : void 0,
		excludeFile: join$1(gitDir, "info", "exclude")
	};
}
/**
...
async function createWorktreeAndFork(host, sessionId, name, config) {
	...
	if ((await worktreeList(host.subprocess, inside.repoRoot, config.gitTimeoutMs)).some((worktree) => worktree.path === plan.worktreePath)) throw new ForkRejection("worktree-exists", `a worktree already exists at "${plan.worktreePath}"`);
	if ((await runGit(host.subprocess, [
		"git",
		"rev-parse",
		"--verify",
		"--quiet",
		`refs/heads/${plan.branch}`
	], inside.repoRoot, config.gitTimeoutMs)).exitCode === 0) throw new ForkRejection("branch-exists", `a branch named "${plan.branch}" already exists`);
	...
	try {
		...
		return {
			...
			childId,
			workspaceId,
			workspaceAttached: childId !== void 0 && workspaceId !== void 0 ? await attachChildToWorkspace(host.workspaceRegistry, workspaceId, childId) : false,
			forked: true
		};
	} catch (error) {
		host.logger?.warn?.(`worktree-jump: fork into "${plan.worktreePath}" failed; the worktree and its workspace remain available: ${error instanceof Error ? error.message : String(error)}`);
		return {
			plan,
			childId: void 0,
			workspaceId,
...
*/
async function startInExistingWorktree(host, sessionId, worktreePath, config) {
	...
	if (!chosen.inside) throw new ForkRejection("not-git-repo", `"${worktreePath}" is not a git working tree`);
	...
	try {
		...
		host.logger?.warn?.(`worktree-jump: fork into "${worktreePath}" failed; the worktree and its workspace remain available: ${error instanceof Error ? error.message : String(error)}`);
		return {
			plan,
			childId: void 0,
			workspaceId,
...
*/
async function ensureWorkspace(registry, worktreePath) {
...
*/
async function attachChildToWorkspace(registry, workspaceId, childId) {
...
function wireErrorOf(error) {
	if (error instanceof ForkRejection) return {
		status: error.code === "session-not-found" ? 404 : error.code === "worktree-exists" ? 409 : error.code === "branch-exists" ? 409 : error.code === "subagent-session" ? 409 : error.code === "no-workspace" ? 409 : error.code === "not-git-repo" ? 409 : 502,
		body: {
			code: error.code,
			message: error.message
		}
	};
	if (error instanceof GitFailureError || error instanceof GitTimeoutError) return {
		status: 502,
		body: {
			code: "git-failed",
			message: error.message
		...
		handler: async (req, res) => {
			...
			try {
				...
				sendJson(res, 200, {
					...
					...outcome.childId !== void 0 ? { sessionId: outcome.childId } : {},
					...outcome.workspaceId !== void 0 ? { workspaceId: String(outcome.workspaceId) } : {},
	...
	const disposeStart = webServer.register({
		...
		handler: async (req, res) => {
			...
			if (!isAbsolute(body.path)) {
				sendJson(res, 400, {
					code: "bad-request",
					message: "path must be an absolute directory"
				});
				return;
			}
			try {
