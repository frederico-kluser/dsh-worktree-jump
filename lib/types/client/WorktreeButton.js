...
export function WorktreeAction(props) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx("div", { onPointerDown: (event) => { event.stopPropagation(); }, onClick: (event) => { event.stopPropagation(); }, style: { position: 'absolute', top: 10, right: 16, zIndex: 100 }, children: _jsx(Tooltip, { label: t('button.tooltip', { repo: repoName }), side: "bottom", children: _jsx(Button, { variant: "outline", size: "sm", icon: _jsx(IconBranchOutline16, { size: 14 }), "aria-label": t('button.aria'), onClick: () => { setDialogOpen(true); }, children: `${t('button.label')} · ${repoName}` }) }) }), _jsx(WorktreeDialog, { open: dialogOpen, onClose: () => { setDialogOpen(false); }, sessionId: sessionId, cwd: cwd, status: status, create: create, openSession: openSession, t: t })] }));
}
/**
...
export function WorktreeDialog(props) {
    ...
    return (_jsxs(Modal, { open: open, onClose: () => { if (phase !== 'creating')
            onClose(); }, title: t('dialog.title'), description: t('dialog.description'), closeLabel: t('dialog.cancel'), footer: (_jsxs(_Fragment, { children: [_jsx(Button, { variant: "ghost", disabled: phase === 'creating', onClick: onClose, children: t('dialog.cancel') }), _jsx(Button, { variant: "primary", disabled: phase === 'creating' || name.trim() === '', onClick: submit, children: phase === 'creating' ? t('dialog.creating') : t('dialog.submit') })] })), children: [_jsx("label", { htmlFor: "dsh-worktree-jump-name", style: styles.fieldLabel, children: t('dialog.name.label') }), _jsx("input", { id: "dsh-worktree-jump-name", autoFocus: true, placeholder: t('dialog.name.placeholder'), value: name, disabled: phase === 'creating', style: styles.fieldInput, onChange: (event) => { setName(event.target.value); }, onKeyDown: (event) => { if (event.key === 'Enter')
                ...
                ? (_jsxs("details", { style: styles.existing, children: [_jsx("summary", { children: `${t('dialog.existing')} (${String(existing.length)})` }), _jsx("ul", { style: styles.existingList, children: existing.map(worktree => (_jsxs("li", { style: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }, children: [_jsx("span", { style: styles.branch, children: worktree.branch }), ' ', _jsx("code", { children: worktree.path })] }, worktree.path))) })] }))
