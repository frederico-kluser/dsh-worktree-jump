...
import { createWorktreeAndFork, repoFacts, startInExistingWorktree } from "./service.js";
import { validateWorktreeName } from "./worktree.js";
import { GitFailureError, GitTimeoutError } from "./git.js";
import { ForkRejection } from "./fork.js";
...
export function parseCreateBody(text) {
    let body;
    try {
    ...
    if (typeof sessionId !== 'string' || sessionId === '' || typeof name !== 'string')
        return null;
    return { sessionId, name };
}
/** Parse one start body: JSON object with non-empty string sessionId and path. */
export function parseStartBody(text) {
    let body;
    try {
        body = JSON.parse(text);
    }
...
export function wireErrorOf(error) {
 ...
 */
export function registerWorktreeRoutes(webServer, deps) {
